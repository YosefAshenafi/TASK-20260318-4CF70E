...
func (s *Server) Run() error {
	...
	recSvc := service.NewRecruitmentService(recRepo, piiCipher, auditSvc, service.WithResumeImportFiles(fileRepo, s.cfg.FileStorageRoot))
	recH := handler.NewRecruitmentHandler(recSvc)
	complianceRepo := repository.NewComplianceRepository(s.db)
	complianceSvc := service.NewComplianceService(complianceRepo, auditSvc, service.WithFileRepository(fileRepo))
	...
	caseH := handler.NewCaseHandler(caseSvc, nil)
	...
	v1 := r.Group("/api/v1")
	{
		...
		authz.Use(middleware.AccessContext(accessSvc))
		{
			...
			authz.GET("/cases/:id/attachments", middleware.RequirePermission("cases.view"), caseH.ListAttachments)
			authz.POST("/cases/:id/attachments", middleware.RequirePermission("cases.manage"), caseH.AttachFile)
			authz.DELETE("/cases/:id/attachments/:fileId", middleware.RequirePermission("cases.manage"), caseH.DetachFile)
